...
class SembastDb {
  ...
  Future deleteDocs(
    ...
    final store = sembast.stringMapStoreFactory.store(collection);
    final docs = await store.find(db, finder: sembast.Finder(filter: sembast.Filter.and(filter)));
    int cnt = 0;
  ...
  Future _deleteDocs(
    ...
    return store.delete(db,
        finder: filterData == null ? null : sembast.Finder(filter: sembast.Filter.and(filters)));
  }

  Future garbageCollect(String collection, {FilterData? filterData}) {
    return _deleteDocs(collection, filterData: filterData?.addEqualTo(_kDeletedFlag, true));
  }
  ...
  Future setList(String collection, dynamic docs, {bool? merge}) async {
    docs = (docs as Iterable).map((d)=>Json.from(d)).toList();
    final store = sembast.stringMapStoreFactory.store(collection);
  ...
  // Stream<Json> colStream({String? collection}) {
  ...
  Stream<List<Json>> queryStream(String collection,
    ...
    return q.onSnapshots(db).map((event) => event.map((e) => e.value).toList()).asBroadcastStream();
  }
  ...
  sembast.QueryRef<String, Json> _query({
    ...
    return store.query(
      ...
      sortOrders: sortOrders
          ...
          ?.map((e) => sembast.SortOrder(e.field, e.ascending, !(e.nullLast ^ e.ascending)))
          .toList(),
    ));
  }
  ...
  List<sembast.Filter> _calcFilters(
    ...
    if (filterData != null) {
      ...
      if (filterData.arrayContainss != null && filterData.arrayContainss!.isNotEmpty) {
