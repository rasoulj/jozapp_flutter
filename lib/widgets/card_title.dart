...
class CardTitle extends StatelessWidget {
  ...
  final bool hasDiv;
  final bool isEmpty;
  const CardTitle({Key? key, this.title, this.hasDiv = true, this.isEmpty = false}) : super(key: key);
  ...
  Widget build(BuildContext context) => title == null
    ...
    child: Row(
      children: [
        ...
        if(isEmpty) const GrayBox(color: AppColors.grey2, height: 17,),
      ],
    ),
  );
}
...
class TitledCard extends StatelessWidget {
  ...
  const TitledCard(
      {Key? key,
        ...
        this.onPressed,
        this.hasDiv = true, this.isEmpty = false})
      : super(key: key);
